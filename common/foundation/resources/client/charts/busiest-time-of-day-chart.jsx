...
export function BusiestTimeOfDayChart({
  ...
  return <ChartLayout title={<Trans message="Busiest time of day" />} className={clsx(className, colSpan, rowSpan, 'compact-scrollbar overflow-x-auto overflow-y-hidden')}>
      <div className="grid w-full grid-cols-[max-content,repeat(12,minmax(66px,1fr))] gap-1">
        <div className="spacer" />
        {hours.map(hour => <div key={hour} className="py-6 text-center text-xs font-semibold">
            {hour}
          </div>)}
        {days.map(day => <Fragment key={day}>
            <div className="flex h-44 items-center pr-10 text-xs font-semibold">
              {formatter.format(date.add({
            days: day
          }).toDate())}
            </div>
            {hours.map(hour => {
          const count = report ? report.datasets[0].data[day].value[hour] : 0;
          return <Tooltip key={hour} label={<span>
                      <Trans message={report?.datasets[0].label ?? 'No data'} values={{
              count
            }} />
                      : {count}
                    </span>}>
                  <div className="h-44 bg-primary opacity-10 transition-opacity" style={{
