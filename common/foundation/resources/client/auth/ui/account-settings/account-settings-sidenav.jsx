...
export let AccountSettingsId = /*#__PURE__*/function (AccountSettingsId) {
...
export function AccountSettingsSidenav() {
  ...
  return <aside className="sticky top-10 hidden flex-shrink-0 lg:block">
      <List padding="p-0">
        {auth.accountSettingsPanels?.map(panel => <Item key={panel.id} icon={<panel.icon viewBox="0 0 50 50" />} panel={panel.id}>
            <Trans {...panel.label} />
          </Item>)}
        <Item icon={<PersonIcon />} panel={p.AccountDetails}>
          <Trans message="Account details" />
        </Item>
        {!allSocialsDisabled && <Item icon={<LoginIcon />} panel={p.SocialLogin}>
        ...
        {api?.integrated && hasPermission('api.access') ? <Item icon={<ApiIcon />} panel={p.Developers}>
...
function Item({
  ...
  return <ListItem startIcon={icon} className={isLast ? undefined : 'mb-10'} onSelected={() => {
