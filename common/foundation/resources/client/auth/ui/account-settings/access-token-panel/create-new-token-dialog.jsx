...
export function CreateNewTokenDialog() {
  ...
  const formNode = <Form form={form} id={formId} onSubmit={values => {
    createToken.mutate(values, {
      ...
      <FormTextField name="tokenName" label={<Trans message="Token name" />} required autoFocus />
    </Form>;
  return <Dialog>
      <DialogHeader>
      ...
      <DialogBody>
        {plainTextToken ? <PlainTextPreview plainTextToken={plainTextToken} /> : formNode}
      </DialogBody>
      <DialogFooter>
        <Button variant="text" onClick={close}>
          <Trans message="Done" />
        </Button>
        {!plainTextToken && <Button variant="flat" color="primary" type="submit" form={formId} disabled={createToken.isPending}>
...
function PlainTextPreview({
  ...
  return <>
      <TextField readOnly value={plainTextToken} autoFocus onClick={e => {
      e.currentTarget.focus();
      e.currentTarget.select();
    }} endAppend={<Button variant="flat" color="primary" onClick={copyToClipboard}>
