...
export function SocialLoginPanel({
  ...
  return <AccountSettingsPanel id={AccountSettingsId.SocialLogin} title={<Trans message="Manage social login" />}>
      <SocialLoginPanelRow icon={<EnvatoIcon viewBox="0 0 50 50" className="border-envato bg-envato text-white" />} service="envato" user={user} />
      <SocialLoginPanelRow icon={<GoogleIcon viewBox="0 0 48 48" />} service="google" user={user} />
      <SocialLoginPanelRow icon={<FacebookIcon className="text-facebook" />} service="facebook" user={user} />
      <SocialLoginPanelRow icon={<TwitterIcon className="text-twitter" />} service="twitter" user={user} />
...
function SocialLoginPanelRow({
  ...
  const username = user?.social_profiles?.find(s => s.service_name === service)?.username;
  if (!social?.[service]?.enable) {
    return null;
  }
  return <div className={clsx('flex items-center gap-14 border-b px-10 py-20', className)}>
      {cloneElement(icon, {
      size: 'xl',
      ...
      <div className="mr-auto overflow-hidden text-ellipsis whitespace-nowrap">
        <div className="overflow-hidden text-ellipsis text-sm font-bold first-letter:capitalize">
          <Trans message=":service account" values={{
          service
        }} />
        </div>
      ...
      <Button disabled={disconnectSocial.isPending} size="xs" variant="outline" color={username ? 'danger' : 'primary'} onClick={async () => {
      if (username) {
        disconnectSocial.mutate({
          ...
          onSuccess: () => {
            ...
            toast(message('Disabled :service account', {
              values: {
                service
              }
            }));
        ...
        if (e?.status === 'SUCCESS') {
          ...
          toast(message('Enabled :service account', {
            values: {
              service
            }
          }));
