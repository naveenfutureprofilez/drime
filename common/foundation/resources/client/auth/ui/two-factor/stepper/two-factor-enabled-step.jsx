...
export function TwoFactorEnabledStep({
  user,
  onDisabled
}) {
  ...
  const actions = <Fragment>
    ...
    })} variant="outline" disabled={isLoading} className="mr-12">
        <Trans message="Regenerate recovery codes" />
      </Button>
      <Button type="submit" variant="flat" color="danger" disabled={isLoading} onClick={() => {
      withConfirmedPassword(() => {
  ...
  return <TwoFactorStepperLayout title={<Trans message="You have enabled two factor authentication." />} description={<Trans message="Store these recovery codes in a secure password manager. They can be used to recover access to your account if your two factor authentication device is lost." />} actions={actions}>
      <div className="mb-16 rounded bg-alt p-14 font-mono text-sm">
        {user.two_factor_recovery_codes?.map(code => <div className="mb-4" key={code}>
            {code}
          </div>)}
      </div>
