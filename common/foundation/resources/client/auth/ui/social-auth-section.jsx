...
export function SocialAuthSection({
  ...
  return <Fragment>
      ...
      <div className={clsx('flex items-center justify-center gap-14', !social?.compact_buttons && 'flex-col')}>
        {social?.google?.enable ? <SocialLoginButton label={googleLabel} icon={<GoogleIcon viewBox="0 0 48 48" />} onClick={() => handleSocialLogin('google')} /> : null}
        {social?.facebook?.enable ? <SocialLoginButton label={facebookLabel} icon={<FacebookIcon className="text-facebook" />} onClick={() => handleSocialLogin('facebook')} /> : null}
        {social?.twitter?.enable ? <SocialLoginButton label={twitterLabel} icon={<TwitterIcon className="text-twitter" />} onClick={() => handleSocialLogin('twitter')} /> : null}
        {social?.envato?.enable ? <SocialLoginButton label={envatoLabel} icon={<EnvatoIcon viewBox="0 0 50 50" className="text-envato" />} onClick={() => handleSocialLogin('envato')} /> : null}
      </div>
      <DialogTrigger type="modal" isOpen={requestingPassword} onOpenChange={setIsRequestingPassword}>
...
function RequestPasswordDialog() {
  ...
  return <Dialog>
      ...
      <DialogBody>
        ...
        <Form form={form} id={formId} onSubmit={payload => {
        connect.mutate(payload);
      }}>
          <FormTextField autoFocus name="password" type="password" required label={<Trans message="Password" />} />
        </Form>
      </DialogBody>
      <DialogFooter>
        <Button variant="text">
        ...
        <Button type="submit" form={formId} variant="flat" color="primary" disabled={connect.isPending}>
...
function SocialLoginButton({
  ...
  if (settings.social?.compact_buttons) {
    return <IconButton variant="outline" aria-label={trans(label)} onClick={onClick}>
        {icon}
      </IconButton>;
  }
  return <Button variant="outline" startIcon={icon} onClick={onClick} className="min-h-42 w-full">
