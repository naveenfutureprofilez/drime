...
export const PermissionSelector = React.forwardRef(({
  valueListKey = 'permissions',
  ...props
}, ref) => {
  const {
    data
  } = useValueLists([valueListKey]);
  ...
  const groupedPermissions = buildPermissionList(permissions, value, showAdvanced);
  ...
  return <Fragment>
      <Accordion variant="outline" ref={ref}>
        ...
        items,
        anyChecked
      }) => <AccordionItem label={<Trans message={prettyName(groupName)} />} key={groupName} startIcon={anyChecked ? <DoneAllIcon size="sm" /> : undefined}>
            <List>
            ...
            return <div key={permission.id}>
              ...
              }} endSection={<Switch tabIndex={-1} checked={isChecked} onChange={() => {}} />} description={<Trans message={permission.description} />}>
                      <Trans message={permission.display_name || permission.name} />
                    </ListItem>
                    {isChecked && <Restrictions permission={permission} onChange={onRestrictionChange} />}
                  </div>;
          })}
            </List>
          </AccordionItem>)}
      </Accordion>
      <Switch className="mt-30" checked={showAdvanced} onChange={e => {
...
function Restrictions({
  ...
  return <div className="px-40 py-20">
      ...
      const description = restriction.description ? <Trans message={restriction.description} /> : undefined;
      if (restriction.type === 'bool') {
        return <Switch description={description} key={restriction.name} className={clsx(!isLast && 'mb-30')} checked={Boolean(restriction.value)} onChange={e => {
          setRestrictionValue(restriction.name, e.target.checked);
        }}>
              {name}
            </Switch>;
      }
      return <TextField size="sm" label={name} description={description} type="number" key={restriction.name} className={clsx(!isLast && 'mb-30')} value={restriction.value || ''} onChange={e => {
        setRestrictionValue(restriction.name, e.target.value === '' ? undefined : parseInt(e.target.value));
      }} />;
    })}
    </div>;
}
export function FormPermissionSelector(props) {
  const {
    field: {
      onChange,
      value = [],
...
export function buildPermissionList(allPermissions, selectedPermissions, showAdvanced) {
  ...
  allPermissions.forEach(permission => {
    ...
    if (index > -1) {
      const mergedPermission = {
        ...permission,
        restrictions: mergeRestrictions(permission.restrictions, selectedPermissions[index].restrictions)
      };
...
function mergeRestrictions(allRestrictions, selectedRestrictions) {
  return allRestrictions?.map(restriction => {
    const selected = selectedRestrictions.find(r => r.name === restriction.name);
    if (selected) {
      return {
        ...restriction,
