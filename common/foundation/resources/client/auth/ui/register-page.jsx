...
export function RegisterPage({
  ...
  const message = <Trans values={{
    a: parts => <Link className={LinkStyle} to="/login">
            {parts}
          </Link>
  }} message="Already have an account? <a>Sign in.</a>" />;
  return <AuthLayout heading={heading} message={message}>
      <StaticPageTitle>
        <Trans message="Register" />
      </StaticPageTitle>
      <Form form={form} onSubmit={async payload => {
      ...
      if (isValid) {
        ...
        <FormTextField className="mb-32" name="email" type="email" disabled={!!searchParamsEmail} label={<Trans message="Email" />} required />
        <FormTextField className="mb-32" name="password" type="password" label={<Trans message="Password" />} required />
        <FormTextField className="mb-32" name="password_confirmation" type="password" label={<Trans message="Confirm password" />} required />
        {auth?.registerFields ? <auth.registerFields /> : null}
        <PolicyCheckboxes />
        <Button className="block w-full" type="submit" variant="flat" color="primary" size="md" disabled={register.isPending || isVerifying}>
          <Trans message="Create account" />
        </Button>
        <SocialAuthSection dividerMessage={social?.compact_buttons ? <Trans message="Or sign up with" /> : <Trans message="OR" />} />
      </Form>
    </AuthLayout>;
}
function PolicyCheckboxes() {
  ...
  return <div className="mb-32">
      {registration.policies.map(policy => <FormCheckbox key={policy.id} name={policy.id} className="mb-4 block" required>
          <Trans message="I accept the :name" values={{
        name: <CustomMenuItem unstyled className={() => LinkStyle} item={policy} />
      }} />
        </FormCheckbox>)}
    </div>;
}