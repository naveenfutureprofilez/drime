...
export function CommentsDatatablePage({
  ...
  const filters = useMemo(() => {
    return CommentsDatatableFilters.filter(f => f.key !== 'commentable_id' || !commentable);
  }, [commentable]);
  const {
    encodedFilters
  ...
  return <div className={clsx(!hideTitle && 'p-12 md:p-24')}>
      <div className={clsx('mb-16')}>
        <StaticPageTitle>
          <Trans message="Comments" />
        </StaticPageTitle>
        {!hideTitle && <h1 className="text-3xl font-light">
            <Trans message="Comments" />
          </h1>}
      </div>
      <div>
        <AnimatePresence initial={false} mode="wait">
          {selectedComments.length ? <SelectedStateDatatableHeader selectedItemsCount={selectedComments.length} actions={<DeleteCommentsButton size="sm" variant="flat" commentIds={selectedComments} />} key="selected" /> : <DataTableHeader filters={filters} searchValue={params.query} onSearchChange={query => setParams({
          ...params,
          query
        })} key="default" />}
        </AnimatePresence>
        ...
        {query.isLoading ? <FullPageLoader className="min-h-200" /> : <div className="rounded border-x border-t">
            {pagination?.data.map(comment => <CommentDatatableItem key={comment.id} comment={comment} isSelected={selectedComments.includes(comment.id)} onToggle={() => toggleComment(comment.id)} />)}
          </div>}

        {(query.isFetched || query.isPlaceholderData) && !pagination?.data.length ? <DataTableEmptyStateMessage className="pt-50" isFiltering={isFiltering} image={publicDiscussionsImage} title={<Trans message="No comments have been created yet" />} filteringTitle={<Trans message="No matching comments" />} /> : undefined}

        <DataTablePaginationFooter className="mt-10" query={query} onPageChange={page => setParams({
        ...params,
        page
