...
export function ImageBackgroundTab({
  ...
  return <div>
      <div className={className}>
        <CustomImageTrigger value={value} onChange={onChange} diskPrefix={diskPrefix} hideFooter={isInsideDialog} />
        {ImageBackgrounds.map(background => <BackgroundSelectorButton key={background.id} onClick={() => onChange?.({
        ...BaseImageBg,
        ...background
      })} isActive={value?.id === background.id} style={{
        ...cssPropsFromBgConfig(background),
        backgroundAttachment: 'initial'
      }} label={<Trans {...background.label} />} />)}
      </div>
      {positionSelector === 'advanced' ? <AdvancedBackgroundPositionSelector value={value} onChange={onChange} /> : <SimpleBackgroundPositionSelector className="mt-20 border-t pt-14" value={value} onChange={onChange} />}
    </div>;
}
function CustomImageTrigger({
  ...
  return <DialogTrigger type="popover" onClose={imageUrl => {
    onChange?.(imageUrl ? {
      ...BaseImageBg,
      backgroundImage: `url(${imageUrl})`
    } : null);
  }}>
      <BackgroundSelectorButton label={<Trans {...BaseImageBg.label} />} isActive={value?.id === BaseImageBg.id && value?.backgroundImage !== 'none'} className="border-2 border-dashed" style={cssPropsFromBgConfig(value)}>
        <span className="inline-block rounded bg-black/20 p-10 text-white">
          <UploadIcon size="lg" />
        </span>
      </BackgroundSelectorButton>
      <CustomImageDialog value={value} diskPrefix={diskPrefix} hideFooter={hideFooter} />
    </DialogTrigger>;
}
export function CustomImageDialog({
  ...
  const defaultValue = !value?.backgroundImage || !value.backgroundImage.includes('url(') ? undefined : urlFromBackgroundImage(value.backgroundImage);
  const form = useForm({
    defaultValues: {
  ...
  return <Dialog size="sm">
      ...
      <DialogBody>
        <Form id={formId} form={form} onSubmit={values => close(values.imageUrl)}>
          <FileUploadProvider>
            <FormImageSelector autoFocus name="imageUrl" diskPrefix={diskPrefix || 'biolinks'} showRemoveButton onChange={hideFooter ? imageUrl => close(imageUrl) : undefined} />
          </FileUploadProvider>
        </Form>
      </DialogBody>
