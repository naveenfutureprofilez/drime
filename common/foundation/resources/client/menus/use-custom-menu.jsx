...
export function useCustomMenu(menuOrPosition) {
  ...
  return useMemo(() => {
    ...
    const menu = typeof menuOrPosition === 'string' ? settings.menus?.find(s => s.positions?.includes(menuOrPosition)) : menuOrPosition;
    const menuItems = [];
    if (menu) {
      for (const item of menu.items) {
        const hasRoles = (item.roles || []).every(a => user?.roles.find(b => b.id === a));
        ...
        if (!item.action || !hasRoles || !hasPermissions || !hasSettings || !subscriptionStatusMatches(item, user)) {
          continue;
        }
        if (user?.id) {
