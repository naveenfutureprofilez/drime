...
export function NotificationSettingsPage() {
  ...
  return <div className="min-h-screen bg-alt">
      ...
      {!isFetched || !data || !selection ? <div className="container mx-auto my-100 flex justify-center">
          <ProgressCircle size="md" isIndeterminate aria-label="Loading subscriptions..." />
        </div> : <div className="container mx-auto my-20 px-10 md:my-40 md:px-20">
          <div className="rounded border bg-paper px-20 pb-30 pt-20">
            {data.subscriptions.map(group => <div key={group.group_name} className="mb-10 text-sm">
                <GroupRow key={group.group_name} group={group} allChannels={data?.available_channels} selection={selection} setSelection={setSelection} />
                {group.subscriptions.map(subscription => <SubscriptionRow key={subscription.notif_id} subscription={subscription} selection={selection} setSelection={setSelection} allChannels={data?.available_channels} />)}
              </div>)}
            <Button className="ml-10 mt-20" variant="flat" color="primary" disabled={updateSettings.isPending} onClick={() => {
...
function GroupRow({
  ...
  const checkboxes = <div className="ml-auto flex items-center gap-40 max-md:hidden">
      ...
      return <Checkbox key={channelName} orientation="vertical" isIndeterminate={someSelected} checked={allSelected} onChange={async e => {
  ...
  return <div className="flex items-center border-b p-10">
...
function SubscriptionRow({
  ...
  return <div className="items-center border-b py-10 pl-8 pr-10 md:flex md:pl-20">
      ...
      <div className="ml-auto flex items-center gap-40">
        {allChannels.map(channelName => <Checkbox key={channelName} orientation="vertical" checked={selection[notifId][channelName]} onChange={async e => {
