...
export function NotificationDialogTrigger({
  ...
  return <DialogTrigger type="popover">
      <IconButton size="md" className={className} badge={hasUnread ? <Badge className="max-md:hidden">
...
export function NotificationsDialog() {
  ...
  return <Dialog>
      <DialogHeader showDivider actions={!hasUnread && notif.subs.integrated && <IconButton className="text-muted" size="sm" elementType={Link} to="/notifications/settings" target="_blank">
              <SettingsIcon />
            </IconButton>} rightAdornment={hasUnread && <Button variant="text" color="primary" size="xs" startIcon={<DoneAllIcon />} onClick={handleMarkAsRead} disabled={markAsRead.isPending} className="max-md:hidden">
...
function DialogContent() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center px-24 py-20">
        <ProgressCircle aria-label="Loading notifications..." isIndeterminate />
      </div>;
  }
  if (!data?.pagination.data.length) {
    return <div className="px-24 py-20">
