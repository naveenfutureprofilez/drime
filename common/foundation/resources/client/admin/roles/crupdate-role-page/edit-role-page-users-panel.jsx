...
const userColumn = {
  ...
  body: user => <NameWithAvatar image={user.image} label={user.name} description={user.email} />
};
const desktopColumns = [userColumn, {
  key: 'first_name',
  allowsSorting: true,
...
export function EditRolePageUsersPanel({
  ...
  if (role.guests || role.type === 'workspace') {
    return <div className="pb-10 pt-30">
        <DataTableEmptyStateMessage image={teamSvg} title={<Trans message="Users can't be assigned to this role" />} />
      </div>;
  }
  return <DataTable endpoint="users" columns={isMobile ? mobileColumns : desktopColumns} queryParams={{
    roleId: `${role.id}`
  }} actions={<AssignUserAction role={role} />} selectedActions={<RemoveUsersAction role={role} />} emptyStateMessage={<DataTableEmptyStateMessage image={teamSvg} title={<Trans message="No users have been assigned to this role yet" />} filteringTitle={<Trans message="No matching users" />} />} />;
}
function AssignUserAction({
  role
}) {
  const addUsers = useAddUsersToRole(role);
  return <DialogTrigger type="modal" onClose={user => {
    if (user) {
      addUsers.mutate({
        userIds: [user.id]
      }, {
...
export function RemoveUsersAction({
  role
}) {
  ...
  return <DialogTrigger type="modal" onClose={isConfirmed => {
    if (isConfirmed) {
      ...
      <ConfirmationDialog title={<Trans message="Remove [one 1 user|other :count users] from “:name“ role?" values={{
      count: selectedRows.length,
      name: role.name
    }} />} body={<Trans message="This will permanently remove the users." />} confirm={<Trans message="Remove" />} isDanger />
    </DialogTrigger>;
}