  ...
  body: locale => {
    return <div className="text-muted">
          <Tooltip label={<Trans message="Translate" />}>
            <IconButton size="md" elementType={Link} to={`${locale.id}/translate`}>
              <TranslateIcon />
            </IconButton>
          </Tooltip>
...
export function LocalizationIndex() {
  return <DataTablePage endpoint="localizations" title={<Trans message="Localizations" />} columns={columnConfig} actions={<Actions />} selectedActions={<DeleteSelectedItemsAction />} emptyStateMessage={<DataTableEmptyStateMessage image={aroundTheWorldSvg} title={<Trans message="No localizations have been created yet" />} filteringTitle={<Trans message="No matching localizations" />} />} />;
}
function Actions() {
  return <Fragment>
...
function RowActionsMenuTrigger({
  ...
  return <MenuTrigger>
      ...
      <Menu>
        <MenuItem value="translate" elementType={Link} to={`${locale.id}/translate`}>
          <Trans message="Translate" />
        </MenuItem>
        <MenuItem value="rename" onSelected={() => openDialog(UpdateLocalizationDialog, {
        localization: locale
      })}>
