...
function Form({
  ...
  return <form className="flex h-full flex-col p-14 md:p-24" onSubmit={e => {
    e.preventDefault();
    updateLocalization.mutate({
      ...
      <Header localization={localization} setLines={setLines} lines={lines} searchQuery={searchQuery} setSearchQuery={setSearchQuery} isLoading={updateLocalization.isPending} />
...
function Header({
  ...
  return <div className="flex-shrink-0">
      <Breadcrumb size="lg" className="mb-16">
        ...
        <BreadcrumbItem>
          <Trans message=":locale translations" values={{
      ...
      <div className="mb-24 flex items-center gap-32 md:gap-12">
        <div className="max-w-440 flex-auto">
          <TextField value={searchQuery} onChange={e => setSearchQuery(e.target.value)} startAdornment={<SearchIcon />} placeholder={trans({
          message: 'Type to search...'
        })} />
        </div>
        <DialogTrigger type="modal" onClose={newTranslation => {
        if (newTranslation) {
          const newLines = {
            ...lines
          };
          ...
          {!isMobile && <Button className="ml-auto" variant="outline" color="primary" startIcon={<AddIcon />}>
              <Trans message="Add new" />
            </Button>}
          <NewTranslationDialog />
        </DialogTrigger>
        <ActionsMenuTrigger locale={localization} />
        <Button variant="flat" color="primary" type="submit" disabled={isLoading}>
          {isMobile ? <Trans message="Save" /> : <Trans message="Save translations" />}
        </Button>
      </div>
    </div>;
}
function LinesList({
  ...
  const filteredLines = useMemo(() => {
    return Object.entries(lines).filter(([id, translation]) => {
      ...
      return !lowerCaseQuery || id?.toLowerCase().includes(lowerCaseQuery) || translation?.toLowerCase().includes(lowerCaseQuery);
    });
  }, [lines, searchQuery]);
  ...
  return <div className="flex-auto overflow-y-auto" ref={ref}>
      <div className="relative w-full" style={{
      height: `${rowVirtualizer.getTotalSize()}px`
    }}>
        {rowVirtualizer.getVirtualItems().map(virtualItem => {
        const [id, translation] = filteredLines[virtualItem.index];
        return <div key={id} className="absolute left-0 top-0 w-full" style={{
          ...
          transform: `translateY(${virtualItem.start}px)`
              ...
              <div className="mb-10 rounded border md:mr-10">
                <div className="flex items-center justify-between gap-24 border-b px-10 py-2">
                  ...
                  <IconButton size="xs" className="text-muted" onClick={() => {
                const newLines = {
                  ...lines
                };
                delete newLines[id];
                ...
                <div>
                  <textarea id={id} name={id} defaultValue={translation} className="block w-full resize-none rounded bg-inherit p-10 text-sm outline-none focus-visible:ring-2" rows={2} onChange={e => {
                const newLines = {
                  ...lines
                };
...
function ActionsMenuTrigger({
  ...
  return <MenuTrigger>
      <IconButton variant="outline" size="sm" color="primary" disabled={uploadFile.isPending}>
        <MoreVertIcon />
      </IconButton>
      <Menu>
        <MenuItem value="download" onSelected={() => downloadFileFromUrl(`api/v1/localizations/${locale.id}/download`)}>
          <Trans message="Download" />
        </MenuItem>
        <MenuItem value="upload" onSelected={async () => {
