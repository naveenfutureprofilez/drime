...
export const OutgoingEmailLogDatatableColumns = [{
  key: 'message_id',
  allowsSorting: true,
  visibleInMode: 'all',
  width: 'flex-3 min-w-200',
  header: () => <Trans message="Subject" />,
  body: item => <NameWithAvatar label={item.subject} description={item.message_id} />
}, {
  key: 'status',
  allowsSorting: true,
  ...
  body: item => {
    switch (item.status) {
      case 'sent':
        return <StatusChip color="positive">
                <Trans message="Sent" />
              </StatusChip>;
      case 'not-sent':
        return <StatusChip color={undefined}>
                <Trans message="Not sent" />
              </StatusChip>;
      case 'error':
        return <StatusChip color="danger">
                <Trans message="Error" />
              </StatusChip>;
    }
  }
...
function PreviewEmailButton({
  ...
  return <DialogTrigger type="modal">
      <Tooltip label={<Trans message="Preview" />}>
        <IconButton size="md" className="text-muted" disabled={rerunCommand.isPending} onClick={() => {
        rerunCommand.mutate({
          id: item.id
        });
      }}>
...
function StatusChip({
  ...
  return <Chip color={color} size="xs" className="w-max min-w-50 text-center">
      {children}
    </Chip>;
}