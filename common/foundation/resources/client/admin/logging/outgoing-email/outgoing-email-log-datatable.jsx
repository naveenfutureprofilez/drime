...
export function OutgoingEmailLogDatatable() {
  return <DataTablePage padding="pt-12 md:pt-24" endpoint="logs/outgoing-email" title={<Trans message="Outgoing email" />} columns={OutgoingEmailLogDatatableColumns} filters={OutgoingEmailLogDatatableFilters} actions={<Actions />} enableSelection={false} emptyStateMessage={<DataTableEmptyStateMessage image={openedImage} title={<Trans message="No outgoing emails have been logged yet" />} filteringTitle={<Trans message="No matching emails" />} />} />;
}
function Actions() {
  return <DataTableAddItemButton elementType="a" href="api/v1/logs/outgoing-email/download" download icon={<DownloadIcon />}>
