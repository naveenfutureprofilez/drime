...
const columnConfig = [{
  ...
  hideHeader: true,
  body: () => <DragHandleIcon className="cursor-pointer text-muted hover:text" />
}, {
  key: 'name',
  header: () => <Trans message="Content item" />,
  visibleInMode: 'all',
  body: item => {
    return <NameWithAvatar image={item.image} label={item.model_type === 'channel' ? <Link className="hover:underline" to={`/admin/channels/${item.id}/edit`} target="_blank">
                {item.name}
              </Link> : item.name} description={item.description} />;
  }
}, {
  key: 'type',
...
export function ChannelContentEditor({
  ...
  const [perPage, setPerPage] = useState(channel.content?.per_page ?? 100);
  const contentType = watch('config.contentType');
  const contentOrder = watch('config.contentOrder');
  const addToChannel = useAddToChannel();
  const query = useChannelContent(channel, {
    loader: 'editChannelPage',
    paginate: 'simple',
    perPage: `${perPage}`
  });
  ...
  const filteredColumns = columnConfig.filter(col => {
    ...
    if (col.key === 'dragHandle' && (contentType !== 'manual' || contentOrder !== 'channelables.order:asc')) {
      return false;
    }
    return true;
  });
  return <div className="mt-40">
      ...
      <Pagination query={query} perPage={perPage} onPageChange={setPerPage} className="mb-24" />
      <Table className="mt-24" columns={filteredColumns} data={pagination?.data || []} meta={query.queryKey} renderRowAs={contentType === 'manual' ? ContentTableRow : undefined} enableSelection={false} hideHeaderRow />
      {!pagination?.data?.length ? noResultsMessage || <IllustratedMessage title={<Trans message="Channel is empty" />} description={contentType === 'manual' ? <Trans message="No content is attached to this channel yet." /> : <Trans message="No content to show for this channel yet." />} image={<SvgImage src={playlist} />} /> : null}
      <Pagination query={query} perPage={perPage} onPageChange={setPerPage} className="mt-24" />
    </div>;
}
function Pagination({
  query,
  ...
  return <div className={clsx('flex items-center justify-between gap-24', className)}>
      <PaginationControls pagination={query.data} type="simple" />
      {query.data?.data.length >= query.data?.per_page && <Select minWidth="min-w-auto" selectionMode="single" disabled={query.isLoading} labelPosition="side" size="xs" label={<Trans message="Per page" />} selectedValue={`${perPage}`} onSelectionChange={value => onPageChange(value)} className="ml-auto">
...
function ContentTableRow({
  ...
  const {
    ...
    onSortEnd: (oldIndex, newIndex) => {
      ...
      const newPagination = queryClient.setQueryData(meta, pagination => {
        if (pagination) {
          pagination = {
            ...pagination,
      ...
      if (newPagination) {
        reorderContent.mutate({
          ...
          ids: newPagination.data.map(item => item.id)
        });
      }
    }
  });
  return <div className={clsx(className, dropPosition === 'before' && 'sort-preview-before', dropPosition === 'after' && 'sort-preview-after')} ref={domRef} {...mergeProps(sortableProps, domProps)}>
      {children}
...
function RemoveItemColumn({
  ...
  return <IconButton size="md" className="text-muted" disabled={removeFromChannel.isPending} onClick={() => {
...
function ContentNotEditableWarning() {
  ...
  return <div className="mb-20 mt-4 flex items-center gap-8">
      <WarningIcon size="xs" />
      <div className="text-xs text-muted">
        {contentType === 'listAll' ? <Trans message="This channel is listing all available content of specified type, and can't be curated manually." /> : null}
        {contentType === 'autoUpdate' ? <Trans message="This channel content is set to update automatically and can't be curated manually." /> : null}
      </div>
    </div>;
}
function UpdateContentButton() {
  ...
  return <Button size="xs" variant="outline" color="primary" startIcon={<RefreshIcon />} onClick={() => {
