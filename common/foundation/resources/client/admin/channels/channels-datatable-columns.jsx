...
export const ChannelsDatatableColumns = [{
  key: 'name',
  allowsSorting: true,
  width: 'flex-3',
  visibleInMode: 'all',
  ...
  body: channel => {
    return <div>
          ...
          {channel.config.adminDescription && <p className="max-w-680 whitespace-normal text-xs text-muted">
  ...
  body: channel => <span className="capitalize">
        {channel.config.contentModel ? <Trans message={channel.config.contentModel} /> : undefined}
      </span>
}, {
  key: 'internal',
  allowsSorting: true,
  ...
  body: channel => channel.updated_at ? <FormattedDate date={channel.updated_at} /> : ''
}, {
  key: 'actions',
  ...
  body: channel => <Link to={`${channel.id}/edit`} className="text-muted">
        <IconButton size="md">
          <EditIcon />
        </IconButton>
      </Link>
...
function ChannelName({
  ...
  return <a className="outline-none hover:underline focus-visible:underline" href={`channel/${channel.slug}`} target="_blank" rel="noreferrer">
      {channel.name}
    </a>;
}
function InternalColumn({
  ...
  const internalLabel = channel.internal ? <Tooltip label={<Trans message="This channel is required for some site functionality to work properly and can't be deleted." />}>
      <div>
  ...
  const isHomepage = homepage?.type === 'channels' && `${homepage.value}` === `${channel.id}`;
