...
const columnConfig = [{
  ...
  body: entry => {
    const owner = entry.users?.find(user => user.owns_entry) ?? entry.users?.[0];
    if (!owner) return null;
    return <NameWithAvatar image={owner.image} label={owner.name} description={owner.email} />;
  }
}, {
  key: 'type',
  ...
  body: entry => <div className="flex items-center gap-12">
  ...
  body: entry => entry.public ? <CheckIcon className="text-positive icon-md" /> : <CloseIcon className="text-danger icon-md" />
}, {
  key: 'file_size',
  allowsSorting: true,
  maxWidth: 'max-w-100',
...
export function FileEntryIndexPage() {
  return <DataTablePage endpoint="file-entries" title={<Trans message="Uploaded files and folders" />} columns={columnConfig} filters={FILE_ENTRY_INDEX_FILTERS} selectedActions={<DeleteSelectedItemsAction />} emptyStateMessage={<DataTableEmptyStateMessage image={uploadSvg} title={<Trans message="Nothing has been uploaded yet" />} filteringTitle={<Trans message="No matching files or folders" />} />} />;
}