...
export function CrupdateResourceLayout({
  ...
  const saveButton = <Button variant="flat" color="primary" type="submit" disabled={isLoading || !isDirty}>
      {submitButtonText ?? <Trans message="Save" />}
    </Button>;
  return <Form onSubmit={onSubmit} onBeforeSubmit={() => form.clearErrors()} form={form} className="relative">
      {variant === 'sticky' && <div ref={sentinelRef} />}
      <CrupdateResourceHeader wrapInContainer={wrapInContainer} startActions={backButton} subTitle={subTitle} endActions={<Fragment>
            {actions}
            {variant === 'sticky' && saveButton}
          </Fragment>} className={clsx(isSticky && 'bg shadow', variant === 'sticky' && 'sticky')}>
        {title}
      </CrupdateResourceHeader>
      <div className={wrapInContainer ? 'container mx-auto px-24 pb-24' : undefined}>
        <div className="rounded">{children}</div>
      </div>
      {variant === 'popup' && <DirtyFormSaveDrawer saveButton={saveButton} isLoading={isLoading} />}
    </Form>;
}
export function CrupdateResourceHeader({
  ...
  return <div className={clsx('top-0 z-10 my-12 transition-shadow md:my-24', className)}>
      <div className={clsx('flex items-center gap-24 py-14 md:items-start', wrapInContainer && 'container mx-auto px-24')}>
        {startActions}
...
export function CrupdateResourceSection({
  ...
  return <section className={clsx(margin)}>
...
export function DirtyFormSaveDrawer({
  ...
  return <AnimatePresence>
      {formState.isDirty && <Fragment>
          <div className="invisible h-92" />
          <m.div key="dirty-panel" initial={{
        y: 100,
        opacity: 0
      }} animate={{
        y: 0,
        ...
        duration: 0.2
            ...
            {saveButton ?? <Button variant="flat" color="primary" type="submit" disabled={isLoading}>
