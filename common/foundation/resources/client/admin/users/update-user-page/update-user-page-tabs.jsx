...
export const updateUserPageTabs = [{
  uri: 'details',
  label: message('Details')
}, {
  uri: 'permissions',
  label: message('Roles & permissions')
}, {
  uri: 'security',
...
export function UpdateUserPageTabs({
  user,
  tabs
}) {
  const {
  ...
  return <div className="container mx-auto px-24">
      <Tabs selectedTab={activeTab} onTabChange={setActiveTab} overflow="overflow-visible">
        <TabList className="mb-24">
          {filteredTabs.map(tab => <Tab key={tab.uri} elementType={Link} to={tab.uri}>
              <Trans {...tab.label} />
            </Tab>)}
        </TabList>
        <Outlet context={user} />
