...
export function UpdateUserPageHeader({
  user,
  badge
}) {
  const isSuspended = user.banned_at !== null;
  const banReason = user.bans?.[0]?.comment;
  return <div className="container mx-auto mb-44 mt-38 px-24">
      <div className="flex gap-32">
        ...
        <div>
          {!!user.roles.length && <Chip radius="rounded-panel" size="sm" className="mb-6 w-max">
              {user.roles[0].name}
            </Chip>}
      ...
      {isSuspended && <div className="mt-24 flex w-max items-center gap-8 rounded-panel bg-danger-lighter px-10 py-6 text-sm text-danger-darker">
          <ErrorOutlineIcon size="sm" />
          {banReason ? <Trans message="Suspended: :reason" values={{
        reason: banReason
...
function AvatarSelector({
  ...
  return <FileUploadProvider>
      <ImageSelector value={value} diskPrefix="avatars" variant="avatar" stretchPreview previewSize="w-90 h-90" placeholderIcon={<Avatar label={user.name} size="w-full h-full text-2xl" circle />} showRemoveButton onChange={url => {
      const onSuccess = {
