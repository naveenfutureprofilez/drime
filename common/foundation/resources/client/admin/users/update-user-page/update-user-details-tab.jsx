...
export function UpdateUserDetailsTab() {
  ...
  return <UpdateUserForm form={form}>
      <div className="mb-24 flex gap-44">
        <FormTextField name="first_name" label={<Trans message="First name" />} className="flex-auto" />
        <FormTextField name="last_name" label={<Trans message="Last name" />} className="flex-auto" />
      </div>
      <FormFileSizeField className="mb-24" name="available_space" label={<Trans message="Allowed storage space" />} description={<Trans values={{
...
function EmailConfirmSection({
  ...
  return <div className="mb-44">
      <FormSwitch className="mb-30" disabled={!require_email_confirmation} name="email_verified_at" description={<Trans message="Whether email address has been confirmed. User will not be able to login until address is confirmed, unless confirmation is disabled from settings page." />}>
        <Trans message="Email confirmed" />
      </FormSwitch>
      <Button size="xs" variant="outline" color="primary" disabled={!require_email_confirmation || resendConfirmationEmail.isPending || !!user.email_verified_at} onClick={() => {
