...
export const userDatatableColumns = [{
  ...
  body: user => <NameWithAvatar image={user.image} label={user.name} description={user.email} alwaysShowAvatar avatarCircle />
}, {
  key: 'roles',
  header: () => <Trans message="Roles" />,
  body: user => <ChipList radius="rounded" size="xs">
        {user?.roles?.map(role => <Chip key={role.id} selectable>
            <Link className={clsx('capitalize')} target="_blank" to={`/admin/roles/${role.id}/edit`}>
  ...
  width: 'w-96',
  body: user => user.subscriptions?.filter(s => s.valid).length ? <CheckIcon className="text-positive icon-md" /> : <CloseIcon className="text-danger icon-md" />
}, {
  key: 'banned_at',
  allowsSorting: true,
  header: () => <Trans message="Suspended" />,
  width: 'w-96',
  body: user => user.banned_at ? <CheckIcon className="text-danger icon-md" /> : null
}, {
  key: 'last_login',
  width: 'w-110',
  header: () => <Trans message="Last active" />,
  body: user => user.last_login ? <time>
  ...
  align: 'end',
  visibleInMode: 'all',
  body: user => <div className="text-muted">
        ...
        {user.banned_at ? <UnbanButton user={user} /> : <DialogTrigger type="modal">
...
function UnbanButton({
  ...
  return <DialogTrigger type="modal" onClose={confirmed => {
    if (confirmed) {
      ...
      <ConfirmationDialog isDanger title={<Trans message="Suspend “:name“" values={{
      name: user.name
    }} />} body={<Trans message="Are you sure you want to remove suspension from this user?" />} confirm={<Trans message="Unsuspend" />} />
    </DialogTrigger>;
}
function ImpersonateButton({
  ...
  return <DialogTrigger type="modal">
      ...
      <ConfirmationDialog title={<Trans message="Login as “:name“" values={{
      name: user.name
    }} />} isLoading={impersonate.isPending} body={<Trans message="Are you sure you want to login as this user?" />} confirm={<Trans message="Login" />} onConfirm={() => {
      impersonate.mutate({
        userId: user.id
      });
    }} />
