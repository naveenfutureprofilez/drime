...
const columnConfig = [{
  ...
  body: product => {
    ...
    return <NameWithAvatar label={product.name} description={product.free ? <Trans message="Free" /> : <FormattedPrice price={price} />} />;
  }
}, {
  key: 'created_at',
  allowsSorting: true,
  ...
  body: product => {
    return <Fragment>
          <IconButton size="md" className="text-muted" elementType={Link} to={`/admin/plans/${product.id}/edit`}>
            <EditIcon />
          </IconButton>
...
export function PlansIndexPage() {
  ...
  return <DataTablePage endpoint="billing/products" title={<Trans message="Subscription plans" />} columns={columnConfig} actions={<Actions />} enableSelection={false} filters={PlansIndexPageFilters} onRowAction={item => {
    navigate(`/admin/plans/${item.id}/edit`);
  }} emptyStateMessage={<DataTableEmptyStateMessage image={softwareEngineerSvg} title={<Trans message="No plans have been created yet" />} filteringTitle={<Trans message="No matching plans" />} />} />;
}
  ...
  return <DialogTrigger type="modal" onClose={confirmed => {
    if (confirmed) {
      ...
      <Tooltip label={<Trans message="Delete plan" />}>
        <IconButton size="md" className="text-muted" disabled={deleteProduct.isPending}>
          <DeleteIcon />
        </IconButton>
      </Tooltip>
      <ConfirmationDialog title={<Trans message="Delete plan" />} body={<Trans message="Are you sure you want to delete this plan?" />} confirm={<Trans message="Delete" />} />
    </DialogTrigger>;
}
function Actions() {
  ...
  return <Fragment>
      <Tooltip label={<Trans message="Sync plans with Stripe & PayPal" />}>
        <IconButton color="primary" variant="outline" size="sm" disabled={syncPlans.isPending} onClick={() => {
        syncPlans.mutate();
      }}>
          <SyncIcon />
        </IconButton>
