...
const columnConfig = [{
  ...
  body: subscription => subscription.user && <NameWithAvatar image={subscription.user.image} label={subscription.user.name} description={subscription.user.email} />
}, {
  key: 'status',
  width: 'w-100 flex-shrink-0',
  header: () => <Trans message="Status" />,
  body: subscription => <Chip size="xs" color={subscription.valid ? 'positive' : 'danger'} radius="rounded" className="w-max">
  ...
  body: subscription => <span className="capitalize">{subscription.gateway_name}</span>
}, {
  key: 'renews_at',
  allowsSorting: true,
...
export function SubscriptionsIndexPage() {
  return <DataTablePage endpoint={endpoint} title={<Trans message="Subscriptions" />} columns={columnConfig} filters={SubscriptionIndexPageFilters} actions={<PageActions />} enableSelection={false} selectedActions={<DeleteSelectedItemsAction />} queryParams={{
    with: 'product'
  }} emptyStateMessage={<DataTableEmptyStateMessage image={subscriptionsSvg} title={<Trans message="No subscriptions have been created yet" />} filteringTitle={<Trans message="No matching subscriptions" />} />} />;
}
...
function SubscriptionActions({
  ...
  return <Fragment>
      ...
      {subscription.cancelled && subscription.on_grace_period ? <ResumeSubscriptionButton subscription={subscription} /> : null}
      {subscription.active ? <SuspendSubscriptionButton subscription={subscription} /> : null}
      <CancelSubscriptionButton subscription={subscription} />
    </Fragment>;
}
function SuspendSubscriptionButton({
  subscription
}) {
  ...
  return <DialogTrigger type="modal" onClose={confirmed => {
    if (confirmed) {
      handleSuspendSubscription();
    }
  }}>
      <Tooltip label={<Trans message="Cancel subscription" />}>
        <IconButton size="md" className="text-muted" disabled={cancelSubscription.isPending}>
          <PauseIcon />
        </IconButton>
      </Tooltip>
      <ConfirmationDialog title={<Trans message="Cancel subscription" />} body={<div>
          ...
          </div>} confirm={<Trans message="Confirm" />} />
    </DialogTrigger>;
}
  ...
  return <DialogTrigger type="modal" onClose={confirmed => {
    if (confirmed) {
      ...
      <Tooltip label={<Trans message="Renew subscription" />}>
        <IconButton size="md" className="text-muted" onClick={handleResumeSubscription} disabled={resumeSubscription.isPending}>
          <PlayArrowIcon />
        </IconButton>
      </Tooltip>
      <ConfirmationDialog title={<Trans message="Resume subscription" />} body={<div>
...
function CancelSubscriptionButton({
  ...
  return <DialogTrigger type="modal" onClose={confirmed => {
    if (confirmed) {
      ...
      <ConfirmationDialog isDanger title={<Trans message="Delete subscription" />} body={<div>
