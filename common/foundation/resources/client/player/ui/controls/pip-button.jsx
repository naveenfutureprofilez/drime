...
export function PipButton({
  ...
  const labelMessage = trans(isPip ? message('Exit picture-in-picture (p)') : message('Enter picture-in-picture (p)'));
  return <Tooltip label={<Trans message={labelMessage} />} usePortal={false}>
      ...
      if (isPip) {
        ...
        {isPip ? <MediaPictureInPictureExitIcon /> : <MediaPictureInPictureIcon />}
      </IconButton>
    </Tooltip>;
}