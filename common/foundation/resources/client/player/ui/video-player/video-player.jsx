...
export function VideoPlayer({
  ...
  return <PlayerContext id={id} options={{
    autoPlay,
    listeners,
    onDestroy,
    onBeforePlayNext,
...
function PlayerLayout({
  ...
  return <div className={clsx('fullscreen-host relative isolate aspect-video bg-black', !controlsVisible && 'cursor-none')} onClick={clickHandler} onPointerEnter={() => {
    setControlsVisible(true);
    clearTimers();
  }} onPointerMove={() => {
    ...
    leaveTimerRef.current = window.setTimeout(() => {
      ...
      <OverlayButtons />
      <BufferingSpinner className="spinner pointer-events-none absolute inset-0 z-40 m-auto h-50 w-50" fillColor="border-white" trackColor="border-white/30" size="w-50 h-50" />
      <BottomGradient />
      <VideoPlayerControls rightActions={rightActions} onPointerEnter={() => {
...
function OverlayButtons() {
  ...
  return <div className={clsx('absolute left-0 top-0 z-40 flex h-full w-full items-center justify-center transition-opacity', showPlayButton ? 'opacity-100' : 'pointer-events-none opacity-0')}>
      <IconButton color="primary" variant="raised" size="lg" radius="rounded-full">
        <MediaPlayIcon />
      </IconButton>
    </div>;
}
...
function BottomGradient() {
  const controlsVisible = usePlayerStore(s => s.controlsVisible);
  return <div className={clsx('player-bottom-gradient pointer-events-none absolute bottom-0 z-30 h-full w-full transition-opacity duration-300', controlsVisible ? 'opacity-100' : 'opacity-0')} />;
}
