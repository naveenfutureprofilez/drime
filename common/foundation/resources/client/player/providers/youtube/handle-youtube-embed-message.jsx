...
export function handleYoutubeEmbedMessage(e, internalStateRef, iframeRef, store) {
  ...
  if (isNumber(info.currentTime) && info.currentTime !== internalState.currentTime) {
    ...
    if (!store.getState().isSeeking) {
      emit('progress', {
        currentTime: internalState.currentTime
      });
    }
  }
  ...
  if (Array.isArray(info.availablePlaybackRates)) {
    emit('playbackRates', {
      rates: info.availablePlaybackRates
    });
  }
...
function onYoutubeStateChange(info, internalStateRef, iframeRef, store) {
  ...
  const onCued = async () => {
    ...
    if (info.videoData?.video_id && !store.getState().cuedMedia?.poster) {
      ...
      if (url) {
        store.getState().emit('posterLoaded', {
          url
        });
      }
    }
