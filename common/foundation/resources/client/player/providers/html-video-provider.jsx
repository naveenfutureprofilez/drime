...
export function HtmlVideoProvider() {
  ...
  return <video className="w-full h-full" ref={ref} src={src} playsInline poster={cuedMedia?.poster} autoPlay={autoPlay} muted={muted} {...events}>
      {cuedMedia?.captions?.map((caption, index) => <track key={caption.id} label={caption.label} kind="subtitles" srcLang={caption.language || 'en'} src={caption.src} default={index === 0} />)}
    </video>;
}