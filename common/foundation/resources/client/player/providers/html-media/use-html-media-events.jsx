...
export function useHtmlMediaEvents({
  ...
  const onTextTracksChange = useCallback(() => {
    ...
    const tracks = Array.from(ref.current.textTracks).filter(t => t.label && (t.kind === 'subtitles' || t.kind === 'captions'));
    let trackId = -1;
    ...
    store.getState().emit('textTrackVisibilityChange', {
      isVisible
    });
    store.getState().emit('textTracks', {
      tracks
    });
  }, [ref, store]);
  useEffect(() => {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      onLoadedMetadata: e => {
        ...
        emit('cued');
        emit('playbackRates', {
          rates: defaultPlaybackRates
        });
      }
    };
  }, [internalState, store, updateCurrentTime, onTextTracksChange, updateBuffered]);
}