...
export function useHtmlMediaInternalState(ref) {
  ...
  const updateCurrentTime = useCallback(() => {
    ...
    if (internalState.current.currentTime !== newTime && !store.getState().isSeeking) {
      store.getState().emit('progress', {
        currentTime: newTime
      });
      internalState.current.currentTime = newTime;
    }
  }, [internalState, store, ref]);
  const toggleTextTrackModes = useCallback((newTrackId, isVisible) => {
