...
export default function HlsProvider() {
  ...
  const setupHls = useCallback(() => {
    ...
    hlsInstance.on(Hls.Events.MANIFEST_PARSED, () => {
      ...
      store.getState().emit('playbackQualityChange', {
        quality: 'auto'
      });
    });
    hlsInstance.on(Hls.Events.AUDIO_TRACK_SWITCHED, (eventType, data) => {
      ...
      if (track) {
        store.getState().emit('currentAudioTrackChange', {
          trackId: track.id
        });
      }
    });
    hlsInstance.on(Hls.Events.LEVEL_LOADED, (eventType, data) => {
      if (!store.getState().providerReady) {
        ...
        store.getState().emit('streamTypeChange', {
          streamType: store.getState().cuedMedia?.streamType || inferredStreamType
        });
        store.getState().emit('durationChange', {
          duration
        });
        const audioTracks = hlsInstance.audioTracks.map(track => ({
          id: track.id,
          label: track.name,
          language: track.lang || '',
          kind: 'main'
        }));
        store.getState().emit('audioTracks', {
          tracks: audioTracks
        });
      }
    });
  ...
  useEffect(() => {
    if (hls.current && cuedMedia?.src && hls.current.url !== cuedMedia?.src) {
    ...
    store.setState({
      providerApi: {
        ...
        setPlaybackQuality: quality => {
          if (!hls.current) return;
          hls.current.currentLevel = hls.current.levels.findIndex(level => levelToPlaybackQuality(level) === quality);
          store.getState().emit('playbackQualityChange', {
            quality
          });
        }
  ...
  return <video className="h-full w-full" ref={videoRef} playsInline poster={cuedMedia?.poster} {...htmlMediaEvents} />;
}
