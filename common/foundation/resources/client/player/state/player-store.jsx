...
import { createFullscreenSlice } from '@common/player/state/fullscreen/fullscreen-slice';
import { createPipSlice } from '@common/player/state/pip/pip-slice';
import { subscribeWithSelector } from 'zustand/middleware';
export const createPlayerStore = (id, options) => {
  // initialData from options should take priority over local storage data
  const initialData = deepMerge(getPlayerStateFromLocalStorage(id, options), options.initialData || {});
  const setInLocalStorage = (key, value) => {
    _setInLocalStorage(`player.${id}.${key}`, value);
  };
  return createStore()(subscribeWithSelector(immer((set, get, store) => {
    ...
    const internalListeners = {
      ...
      currentTextTrackChange: ({
        trackId
      }) => {
        set({
          currentTextTrack: trackId
        });
      },
      textTrackVisibilityChange: ({
        isVisible
      }) => {
        set({
          textTrackIsVisible: isVisible
        });
      },
      buffering: ({
        isBuffering
    ...
    return {
      ...
      appendToQueue: (mediaItems, afterCuedMedia = true) => {
        ...
        set(s => {
          ...
          s.originalQueue = prependToArrayAtIndex(s.originalQueue, mediaItems, index);
        });
        if (options.persistQueueInLocalStorage) {
          setInLocalStorage('queue', get().originalQueue.slice(0, 15));
        }
      ...
      removeFromQueue: mediaItems => {
        set(s => {
          s.shuffledQueue = s.shuffledQueue.filter(item => !mediaItems.find(m => isSameMedia(m, item)));
          s.originalQueue = s.originalQueue.filter(item => !mediaItems.find(m => isSameMedia(m, item)));
        });
        if (options.persistQueueInLocalStorage) {
          setInLocalStorage('queue', get().originalQueue.slice(0, 15));
        }
