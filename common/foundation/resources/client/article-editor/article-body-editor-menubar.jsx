...
export function ArticleBodyEditorMenubar({
  ...
  return <div className={clsx(extendedVisible ? 'h-84' : 'h-42')}>
      <div className={clsx(MenubarRowClassName, justify, 'relative z-20')}>
        ...
        <Divider />
        {isMobile ? <IconButton className="flex-shrink-0" color={extendedVisible ? 'primary' : null} size={size} onClick={() => {
        setExtendedVisible(!extendedVisible);
      }}>
            {extendedVisible ? <UnfoldLessIcon /> : <UnfoldMoreIcon />}
          </IconButton> : <ExtendedButtons editor={editor} size={size} hideInsertButton={hideInsertButton} imageDiskPrefix={imageDiskPrefix} />}
      </div>
      <AnimatePresence>
        {extendedVisible && <m.div className={clsx(MenubarRowClassName, justify, 'absolute flex h-full w-full')} initial={{
        y: '-100%'
      }} animate={{
        y: 0
      }} exit={{
        y: '-100%'
            ...
            <ExtendedButtons editor={editor} size={size} imageDiskPrefix={imageDiskPrefix} />
          </m.div>}
      </AnimatePresence>
    </div>;
}
