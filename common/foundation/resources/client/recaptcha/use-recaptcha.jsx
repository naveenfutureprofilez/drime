...
async function execute(siteKey, action) {
  ...
  return new Promise(resolve => {
    window.grecaptcha?.ready(async () => {
      ...
      const result = apiClient.post('recaptcha/verify', {
        token
      }).then(r => r.data.success).catch(() => false);
      resolve(result ?? false);
    });
  });
}
function load(siteKey) {
  return lazyLoader.loadAsset(`https://www.google.com/recaptcha/api.js?render=${siteKey}`, {
    id: 'recaptcha-js'
  });
}