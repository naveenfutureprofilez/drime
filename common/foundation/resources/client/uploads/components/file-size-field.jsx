...
export const FormFileSizeField = React.forwardRef(({
  ...
  const formProps = {
    onChange: e => {
      ...
      if (Number.isNaN(value)) {
        ...
        const newBytes = convertToBytes(parseInt(e.target.value), unit);
  ...
  const unitSelect = <Select minWidth="min-w-80" selectionMode="single" selectedValue={unit} disabled={!byteValue} inputRadius="rounded-l-none rounded-r-input" onSelectionChange={newUnit => {
    const newBytes = convertToBytes(liveValue || 0, newUnit);
    setByteValue(newBytes);
  }}>
      {spaceUnits.slice(0, 5).map(u => <Option value={u} key={u}>
          {u === 'B' ? 'Bytes' : u}
        </Option>)}
    </Select>;
  return <TextField {...mergeProps(formProps, props)} type="number" ref={ref} endAppend={unitSelect} />;
});
