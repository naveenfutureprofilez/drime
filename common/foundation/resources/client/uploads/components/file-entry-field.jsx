import React, { cloneElement, useCallback, useId, useRef } from 'react';
...
export function FileEntryField({
  ...
  const uploadOptions = {
    ...
    onSuccess: entry => onChange?.(entry.url),
    onError: message => {
      if (message) {
  ...
  const removeButton = showRemoveButton ? <Button variant="link" color="danger" size="xs" disabled={isDeletingEntry || !currentValue || disabled} onClick={() => {
    deleteEntry({
      onSuccess: () => onChange?.('')
    });
  }}>
  ...
  return <div className={clsx('text-sm', className)}>
      {label && <div className="flex items-center justify-between gap-24">
      ...
      <div aria-labelledby={labelId} aria-describedby={descriptionId}>
        <Field fieldClassNames={inputFieldClassNames} errorMessage={errorMessage} invalid={invalid}>
          <FileInputField inputFieldClassNames={inputFieldClassNames} currentValue={currentValue} currentEntry={currentEntry} handleUpload={handleUpload}>
          ...
          required={currentValue ? false : required} accept={allowedFileTypes?.join(',')} type="file" disabled={uploadStatus === 'inProgress'} className="sr-only" onChange={e => {
            if (e.target.files?.length) {
              ...
              const errorMessage = validateFile(new UploadedFile(e.target.files[0]), uploadOptions.restrictions);
    ...
    return <Field wrapperProps={{
      onClick: () => {
        ...
        <Input className={clsx(inputFieldClassNames.input, 'gap-10')}>
          <button ref={buttonRef} type="button" className="flex-shrink-0 rounded bg-primary px-10 py-2 text-sm font-semibold text-on-primary outline-none" onClick={() => handleUpload()}>
            <Trans message="Replace file" />
          </button>
          <AnimatePresence initial={false} mode="wait">
            <div className="min-w-0 overflow-hidden overflow-ellipsis whitespace-nowrap">
              {currentEntry ? <m.div key="file-entry-name" {...opacityAnimation}>
                  {currentEntry.name}
                </m.div> : <m.div key="skeleton" {...opacityAnimation}>
...
export function FormFileEntryField(props) {
  const {
    field: {
      onChange,
      value = null
