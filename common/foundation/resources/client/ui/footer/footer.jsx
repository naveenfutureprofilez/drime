...
export function Footer({
  ...
  return <footer className={clsx('text-sm', padding ? padding : 'pb-28 pt-54 md:pb-54', className)}>
      <Menus />
      <div className="items-center justify-between gap-30 text-center text-muted md:flex md:text-left">
        <Trans message="Copyright © :year :name, All Rights Reserved" values={{
        year,
...
function Menus() {
  ...
  return <div className="mb-14 items-center justify-between gap-30 overflow-x-auto border-b pb-14 md:flex">
      {primaryMenu && <CustomMenu menu={primaryMenu} className="text-primary" />}
      {secondaryMenu && <CustomMenu menu={secondaryMenu} className="mb:mt-0 mt-14 text-muted" />}
    </div>;
}
function ThemeSwitcher() {
  ...
  return <Button variant="text" startIcon={selectedTheme.is_dark ? <DarkModeIcon /> : <LightbulbIcon />} onClick={() => {
    if (selectedTheme.is_dark) {
      ...
      {selectedTheme.is_dark ? <Trans message="Light mode" /> : <Trans message="Dark mode" />}
    </Button>;
}