...
import { NavbarAuthUser } from '@common/ui/navigation/navbar/navbar-auth-user';
...
export function Navbar(props) {
  ...
  darkModeColor = darkModeColor ?? darkThemeVars?.['--be-navbar-color'] ?? 'bg-alt';
  if (isDarkMode) {
    color = darkModeColor;
  }
  return <div style={alwaysDarkMode ? darkThemeVars : undefined} className={clsx(getColorStyle(color, textColor), size === 'md' && 'h-64 py-8', size === 'sm' && 'h-54 py-4', size === 'xs' && 'h-48 py-4', border, className)}>
      <div className={clsx('flex h-full items-center justify-end gap-10 pl-14 pr-8 md:pl-20 md:pr-20', wrapInContainer && 'container mx-auto')}>
        {!hideLogo && <Logo size="h-full max-h-26 md:max-h-36" className="mr-4 md:mr-24" color={resolveLogoColor({
        navbarColor: color,
        logoColor,
        isDarkMode
      })} />}
        ...
        <div className="ml-auto flex items-center gap-4 md:gap-14">
          ...
          {isLoggedIn ? <NavbarAuthUser variant={isMobile ? 'compact' : 'wide'} items={authMenuItems} /> : <NavbarAuthButtons navbarColor={color} primaryButtonColor={primaryButtonColor} />}
        </div>
      </div>
    </div>;
}
function DesktopMenu({
  ...
  return <CustomMenu className="mx-14 text-sm max-md:hidden" itemClassName={isActive => clsx('opacity-90 hover:underline hover:opacity-100', isActive && 'opacity-100')} menu={position} />;
}
function MobileMenu({
  position
}) {
  ...
  return <MenuTrigger>
      ...
      <Menu>
        ...
        return <Item value={item.action} onSelected={() => handleItemClick(item)} key={item.id} startIcon={Icon && <Icon />}>
