...
export function NavbarAuthMenu({
  ...
  const notifMenuItem = <MenuItem className="md:hidden" value="notifications" startIcon={<NotificationsIcon />} onSelected={() => {
    navigate('/notifications');
  }}>
      <Trans message="Notifications" />
      {hasUnreadNotif ? ` (${user.unread_notifications_count})` : undefined}
    </MenuItem>;
  const billingMenuItem = <MenuItem value="billing" startIcon={<PaymentsIcon />} onSelected={() => {
    navigate('/billing');
  }}>
      <Trans message="Billing" />
    </MenuItem>;
  return <MenuTrigger placement={placement}>
      {children}
      <Menu>
        {menu && menu.items.map(item => {
        const Icon = item.icon && createSvgIconFromTree(item.icon);
        return <MenuItem value={item.id} key={item.id} startIcon={Icon && <Icon />} onSelected={() => {
          if (item.type === 'link') {
        ...
        {auth.getUserProfileLink && <MenuItem value="profile" startIcon={<AccountCircleIcon />} onSelected={() => {
        ...
        {themes?.user_change && !selectedTheme.is_dark && <MenuItem value="light" startIcon={<DarkModeIcon />} onSelected={() => {
        selectTheme('dark');
      }}>
            <Trans message="Dark mode" />
          </MenuItem>}
        {themes?.user_change && selectedTheme.is_dark && <MenuItem value="dark" startIcon={<LightModeIcon />} onSelected={() => {
        selectTheme('light');
      }}>
            <Trans message="Light mode" />
          </MenuItem>}
        <MenuItem value="logout" startIcon={<ExitToAppIcon />} onSelected={() => {
        logout.mutate();
      }}>
