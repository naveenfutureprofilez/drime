...
export function NavbarAuthUser({
  ...
  const avatar = <UserAvatar user={user} circle fallback="initials" size={variant === 'wide' ? wideAvatarSize : compactAvatarSize} />;
  const compactButton = <ButtonBase aria-label="toggle authentication menu" display="block">
      {avatar}
      {hasUnreadNotif ? <Badge>{user.unread_notifications_count}</Badge> : undefined}
    </ButtonBase>;
  const wideButton = <ButtonBase className="flex items-center">
      {avatar}
  ...
  return <NavbarAuthMenu items={items}>
