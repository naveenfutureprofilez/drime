...
export function Breadcrumb(props) {
  ...
  if (childArray.length > visibleItems) {
    ...
    const menuItem = <BreadcrumbItem key="menu" sizeStyle={style} isMenuTrigger>
        <MenuTrigger selectionMode="single" selectedValue={selectedKey}>
          ...
          <Menu>
            ...
            return <MenuItem key={index} value={index} onSelected={() => {
              if (!isLast) {
  ...
  return <Element className={clsx(className, 'w-full min-w-0')} // prevent flex parent overflow
  aria-label={trans({
    message: 'Breadcrumbs'
  })} ref={domRef}>
      <ol ref={listRef} className={clsx('flex flex-nowrap justify-start', style.minHeight)}>
        {breadcrumbItems}
      </ol>
    </Element>;
}
