...
export const UpsellLabel = memo(({
  ...
  return <Fragment>
      <span className="font-medium text-positive-darker">
        {' '}
        (
        <Trans message="Save up to :percentage%" values={{
        percentage: upsellPercentage
      }} />
        )
      </span>
    </Fragment>;
...
function calcHighestUpsellPercentage(products) {
  ...
  const decreases = products.map(product => {
    ...
    const savingsPercentage = Math.round((monthlyAmount - yearlyAmount) / monthlyAmount * 100);
