...
export function ConnectDomainDialog({
  ...
  const stepper = useConnectDomainStepper({
    showGlobalField
  });
  ...
  return <Dialog>
      ...
      <DialogBody>
        <Form form={stepper.form} id={formId} onSubmit={() => {
        stepper.goToNextStep();
      }}>
          <StepComponent stepper={stepper} />
        </Form>
      </DialogBody>
      <DialogFooter startAction={<Button variant="text" onClick={() => {
      close();
    }}>
            <Trans message="Cancel" />
          </Button>}>
        {stepper.hasPreviousStep && <Button startIcon={<KeyboardArrowLeftIcon />} color="primary" variant="text" onClick={() => {
        stepper.goToPreviousStep();
      }} disabled={stepper.state.isLoading}>
            <Trans message="Previous" />
          </Button>}
        <Button variant="flat" color="primary" type="submit" form={formId} endIcon={<KeyboardArrowRightIcon />} disabled={stepper.state.isLoading}>
          <Trans message="Next" />
        </Button>
      </DialogFooter>
    </Dialog>;
