...
export function ValidationFailedStep({
  ...
  const errorMessage = validationFailReason === 'serverNotConfigured' && hasPermission('admin') ? <ErrorMessage>
        <Trans message="DNS records for the domain are setup, however it seems that your server is not configured to handle requests from “:host“" values={{
      host: location
    }} />
      </ErrorMessage> : <ErrorMessage>
        <Trans message="The domain is missing :record record pointing to :location or the changes haven't propagated yet." values={{
      record,
      location
    }} />
      </ErrorMessage>;
  return <Fragment>
      ...
      <div className="mt-10 whitespace-nowrap text-xs text-muted">
        <Trans message="You can wait and try again later, or <b>refresh</b>" values={{
        b: text => <button disabled={isLoading} type="button" className="text-primary underline" onClick={() => {
          goToNextStep();
        }}>
                {text}
              </button>
...
function ErrorMessage({
  ...
  return <div className="flex items-center gap-12 rounded bg-warning/15 p-12 text-base font-medium text-warning">
      <WarningIcon size="lg" />
      {children}
    </div>;
}