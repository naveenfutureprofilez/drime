...
export function InsertMenuTrigger({
  ...
  return <>
      ...
      if (key === 'hr') {
        ...
        editor.commands.focus();
        editor.commands.addInfo({
          type: key
        });
      }
    }}>
        <IconButton variant="text" size={size} className={clsx('flex-shrink-0')}>
          <MoreVertIcon />
        </IconButton>
        <Menu>
      ...
      <DialogTrigger type="modal" isOpen={!!dialog} onClose={() => {
      setDialog(false);
    }}>
...
function EmbedDialog({
  ...
  return <Dialog>
      ...
      <DialogBody>
        <Form form={form} id={formId} onSubmit={value => {
        editor.commands.setEmbed(value);
        close();
      }}>
          <FormTextField name="src" label={<Trans message="Embed URL" />} autoFocus type="url" required />
        </Form>
      </DialogBody>
      <DialogFooter>
        ...
        <Button type="submit" form={formId} disabled={!form.formState.isValid} variant="flat" color="primary">
          <Trans message="Add" />
        </Button>
      </DialogFooter>
    </Dialog>;
